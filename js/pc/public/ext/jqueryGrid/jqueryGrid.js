	...
	editGridRow : function(rowid, p){
		...
		return this.each(function(){
			...
			$("#sData", frmtb+"_2").click(function(){				
				...
				*/
				// add 도 checkOnSubmit 가능하게 하고 바뀐게 없을 때는 그냥 닫히게 변경 위에꺼 주석처리(By HoJae)
	...
	navButtonAdd : function (elem, p) {
		...
		return this.each(function() {
			...
			if (findnav) {
				...
				.click(function(e){					
					if (!$(this).hasClass('ui-state-disabled')) {
						// $.debug(jsFileNameStr, printStackTrace(), 'jqueryGrid click', $(this).attr('id'));
						if($(this).attr('id').indexOf('_ilsave') !== -1 || $(this).attr('id').indexOf('_ilcancel') !== -1) {
							var _confirmMsgStr = '';
							/*
							if($(this).attr('id').indexOf('_ilsave') !== -1) {								
								_confirmMsgStr = '수정을 하시겠습니까?';								
							}
							if($(this).attr('id').indexOf('_ilcancel') !== -1) {
								_confirmMsgStr = '취소을 하시겠습니까?';
							}
							*/
							_confirmMsgStr = p.title + '?';
							if(confirm(_confirmMsgStr)) {
								if ($.isFunction(p.onClickButton) ) {p.onClickButton.call($t,e);}
							}
						} else {
							if($(this).attr('id').indexOf('_iledit') !== -1) {
								var sr = $t.p.selrow;
								if (sr) {
									if ($.isFunction(p.onClickButton) ) {p.onClickButton.call($t,e);}
								} else {
									var alertIDs = {themodal: 'alertmod_' + $t.p.id, modalhead: 'alerthd_' + $t.p.id,modalcontent: 'alertcnt_' + $t.p.id};
									$.jgrid.viewModal("#"+alertIDs.themodal,{gbox:"#gbox_"+$.jgrid.jqID($t.p.id),jqm:true});
									$("#jqg_alrt").focus();
								}
							} else {
					...
					if (e.keyCode === 9)  {
						if(!$t.grid.hDiv.loading ) {
							...
							/// Tab
							  ...
							  // 요금관리만 Tab키 누를 때 2칸 이동 By HoJae
							  if(gm.addOnUtil.getCurrentHashUrlStr() === '#/pc/private/manageCash/manageCash') {
    						     $($t).jqGrid("nextCell",iRow,iCol+1);
							  } else {							      
							     $($t).jqGrid("nextCell",iRow,iCol);
							  }
							  // //요금관리만 Tab키 누를 때 2칸 이동 By HoJae
...
$.jgrid.extend({
    ...
    groupingSelect : function(hid){
        this.each(function(){
            ...
            if( tarspan.hasClass(groupUnCheck) ) {
                if(grp.showSummaryOnHide) {
                    if(r){
                        while(r) {
                            ...
                            // $(r).hide();
                            $(r).find('input[type="checkbox"]:first').removeAttr('checked');
                            r = r.nextSibling;
                            if(frz) {
                                // $(r2).hide();
                                $(r2).find('input[type="checkbox"]:first').removeAttr('checked');
                                r2 = r2.nextSibling;
                            }
                        }
                    }
                ...
                tarspan.removeClass(groupUnCheck).addClass(groupCheck);
                // collapsed = true;
            } else {
                if(r){
                    showData = undefined;
                    while(r) {
                        ...
                        if (itemGroupingLevel !== undefined) {
                            ...
                            if (itemGroupingLevel === num + 1) {
                                // $(r).show().find(">td>span."+"tree-wrap-"+$t.p.direction).removeClass(groupUnCheck).addClass(groupCheck);
                                $(r).find('input[type="checkbox"]:first').attr('checked','true');
                                if(frz) {
                                    // $(r2).show().find(">td>span."+"tree-wrap-"+$t.p.direction).removeClass(groupUnCheck).addClass(groupCheck);
                                    $(r2).find('input[type="checkbox"]:first').attr('checked','true');
                                }
                            }
                        } else if (showData) {                            
                            // $(r).show();
                ...
                tarspan.removeClass(groupCheck).addClass(groupUnCheck);
            }
